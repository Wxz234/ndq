...
namespace ndq
{
    ...
    public:
        ...
        virtual std::shared_ptr<IGraphicsBuffer> AllocateBuffer(const NDQ_BUFFER_DESC* pDesc, NDQ_RESOURCE_HEAP_TYPE type, NDQ_RESOURCE_STATE state) = 0;
        virtual std::shared_ptr<IGraphicsTexture2D> AllocateTexture2D(const NDQ_TEXTURE2D_DESC* pDesc, NDQ_RESOURCE_HEAP_TYPE type, NDQ_RESOURCE_STATE state) = 0;
        ...
        virtual std::shared_ptr<IInputLayout> CreateInputLayout(const NDQ_INPUT_ELEMENT_DESC* pInputElementDescs, uint32 numElements) = 0;
    };
